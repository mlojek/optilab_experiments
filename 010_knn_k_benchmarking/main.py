"""
Benchmarking of KNN-IPOP-CMA-ES with different K values on CEC2013/CEC2017 benchmark.
"""
...
from optilab.optimizers import IpopCmaEs, KnnIpopCmaEs
...
if __name__ == "__main__":
    ...
    BUFFER_SIZE = 30 * POPSIZE
    K_VALUES = [DIM, 2 * DIM, 3 * DIM, 5 * DIM, 10 * DIM]
    NUM_RUNS = 51
    CALL_BUDGET = 1e4 * DIM
    TOL = 1e-8
    ...
    for func in FUNCS[args.year]:
        ...
        # KNN-IPOP-CMA-ES with different K values
        for k in K_VALUES:
            knn_optimizer = KnnIpopCmaEs(POPSIZE, k, BUFFER_SIZE)
        ...
        dump_to_pickle(
            results,
            f"010_knn_k_{func.metadata.name}_{DIM}.pkl",
